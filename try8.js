...
function convertFigmaRadioButtonToUnify(figmaJson, overrides = {}) {
  ...
  const strokeColor = checkboxBase.strokes?.[0]?.color || { r: 0, g: 0, b: 0, a: 0 };
  ...
  const borderHex = rgbaToHex(strokeColor.r, strokeColor.g, strokeColor.b, strokeColor.a) || "border-transparent";

  // Extract properties
  const size = getPropertyValue(props, "Size", "md").toLowerCase();
  const isDisabled = getPropertyValue(props, "State") === "Disabled";
  const label = overrides.label || textNode.characters || getPropertyValue(props, "Label", "New Radio Button");
  const description = overrides.description || supportingTextNode.characters || getPropertyValue(props, "Description", "Hello world");
...
const FIGMA_URL = 'https://www.figma.com/design/4r7C2sI9cktH4T8atJhmrW/Component-Sheet?node-id=1-5777&t=Br3U1RuFVDcShCrR-4';
